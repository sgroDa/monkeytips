...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 13002", 0 };
...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 10 shells = 0.1 dicK */
  if (cmd.fee == 10) {
    feeString = "0.1 dicK (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " dicK";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " dicK, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " dicK" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string dicK = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << dicK << " dicK belongs to you!";
    }
  }
  ...
  {
    std::string dicK = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << dicK << " dicK were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "monkeytips v" << PROJECT_VERSION << " Poolwallet" << std::endl;
