  ...
  try {
    ...
    if (o_benchmark)
    {
      ...
      if (rawData.size() >= 43)
      {
        ...
        std::cout << "cn_slow_hash_v6: " << (o_iterations / std::chrono::duration_cast<std::chrono::seconds>(elapsedTime).count()) << " H/s\n";
        ...
        elapsedTime = std::chrono::high_resolution_clock::now() - startTimer;
        std::cout << "cn_slow_hash_v7: " << (o_iterations / std::chrono::duration_cast<std::chrono::seconds>(elapsedTime).count()) << " H/s\n";
      }
