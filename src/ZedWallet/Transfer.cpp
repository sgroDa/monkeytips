             ...
             CryptoNote::TransactionParameters p)
{
    ...
    for (int numTxMultiplier = 2; ; numTxMultiplier++)
    {
        ...
        /* Split the requested fee over each transaction, i.e. if a fee of 200
           dicK was requested and we split it into 4 transactions each one will
           have a fee of 5 dicK. If the fee per transaction is less than the
            ...
            switch (e.code().value())
            {
                ...
                case NodeErrors::CryptoNote::error::CONNECT_ERROR:
                {
                    std::cout << WarningMsg("Couldn't connect to the network "
                              ...
                              << std::endl
                              << "Ensure monkeytipsd or the remote node you "
...
Maybe<uint64_t> getTransferAmount()
{
    while (true)
    {
        ...
        std::cout << std::endl
                  << InformationMsg("How much dicK do you want to send?: ");
...
bool parseFee(std::string feeString)
{
    ...
    {
        std::cout << WarningMsg("Fee must be at least 0.1 dicK!") << std::endl;
        return false;
    }
...
bool parseAddress(std::string address)
{
    ...
    /* We can't get the actual prefix if the address is invalid for other
       ...
       with dicK, aslong as the prefix is the dicK prefix. This keeps it
       working on testnets with different prefixes. */
    else if (expectedPrefix == 3914525 && address.substr(0, 4) != "dicK")
    {
        std::cout << WarningMsg("Invalid address! It should start with dicK!")
                  << std::endl << std::endl;
        return false;
    }
...
bool parseAmount(std::string amountString)
{
    ...
    if (!parseAmount(amountString, amount))
    {
        std::cout << WarningMsg("Failed to parse amount! Ensure you entered "
                                "the value correctly.")
                  << std::endl
                  << "Please note, the minimum you can send is 0.01 dicK,"
                  << std::endl
