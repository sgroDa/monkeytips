                           ...
                           bool fetchTimestamp)
{
    ...
    if (fetchTimestamp)
    {
        ...
        /* Couldn't get timestamp, maybe old node or monkeytipsd closed */
        if (blockTime != "")
        {
...
void saveCSV(CryptoNote::WalletGreen &wallet, CryptoNote::INode &node)
{
    ...
    for (size_t i = 0; i < numTransactions; i++)
    {
        ...
        if (t.totalAmount != 0) {
            ...
            if (t.totalAmount < 0)
            {
                /* Put dicK in separate field, makes output more usable in spreadsheet */
                std::string splitAmtdicK = formatAmount(-t.totalAmount);
                boost::replace_all(splitAmtdicK, " ", ",");
                myfile << "-" << splitAmtdicK << ",OUT\n";
            }
            else
            {
                std::string splitAmtdicK = formatAmount(t.totalAmount);
                boost::replace_all(splitAmtdicK, " ", ",");
                myfile << splitAmtdicK << ",IN\n";
            }
        }
    }
    myfile.close();
                ...
                std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    {
        std::cout << "Your monkeytipsd isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting monkeytipsd.\n"
                    "If this persists, visit "
                    "https://discord.gg/EAaKcCy for support.";
