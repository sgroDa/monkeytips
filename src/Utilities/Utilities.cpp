...
uint64_t scanHeightToTimestamp(const uint64_t scanHeight)
{
    ...
    uint64_t timestamp = CryptoNote::GENESIS_BLOCK_TIMESTAMP
                       + secondsSinceLaunch;
...
uint64_t timestampToScanHeight(const uint64_t timestamp)
{
    ...
    if (timestamp <= CryptoNote::GENESIS_BLOCK_TIMESTAMP)
    {
        return 0;
    }

    /* Find the amount of seconds between launch and the timestamp */
    uint64_t launchTimestampDelta = timestamp - CryptoNote::GENESIS_BLOCK_TIMESTAMP;
